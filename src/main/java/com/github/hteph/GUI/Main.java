			...
			for (int i = 0; i < newSystems.size(); i++) {
				...
				for (int n = 0; n < centralStar.getOrbitalObjects().size(); n++) {
					...
					if(j==0) {	
						...
						String objectName = CentralRegistry.getFromArchive(((Planet) thingOrbitingStar).getLunarObjects().get(j-1)).getName();
						moon.setText(objectName);
						moonbox.getChildren().add(new Label(objectName));
						
						moonbox.getChildren().add(Pagemaker.generator(CentralRegistry.getFromArchive(((Planet) thingOrbitingStar).getLunarObjects().get(j-1))));
					}
